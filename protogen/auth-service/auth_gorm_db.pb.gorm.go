...
type AuthenticationORM struct {
	...
	IsQlola            bool       `gorm:"column:is_qlola;type:boolean"`
	LastChangePassword *time.Time `gorm:"column:last_change_password"`
	Password           string     `gorm:"column:password"`
	PrevSalt           string     `gorm:"column:prev_salt;type:text"`
...
func (m *Authentication) ToORM(ctx context.Context) (AuthenticationORM, error) {
	...
	to.IsQlola = m.IsQlola
	if m.LastChangePassword != nil {
		t := m.LastChangePassword.AsTime()
		to.LastChangePassword = &t
	}
	to.PrevSalt = m.PrevSalt
...
func (m *AuthenticationORM) ToPB(ctx context.Context) (Authentication, error) {
	...
	to.IsQlola = m.IsQlola
	if m.LastChangePassword != nil {
		to.LastChangePassword = timestamppb.New(*m.LastChangePassword)
	}
	to.PrevSalt = m.PrevSalt
...
// TableName overrides the default tablename generated by GORM
func (LoginLogsORM) TableName() string {
	return "login_logs"
}

// ToORM runs the BeforeToORM hook if present, converts the fields of this
// object to ORM format, runs the AfterToORM hook, then returns the ORM object
func (m *LoginLogs) ToORM(ctx context.Context) (LoginLogsORM, error) {
	to := LoginLogsORM{}
	var err error
	if prehook, ok := interface{}(m).(LoginLogsWithBeforeToORM); ok {
		if err = prehook.BeforeToORM(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.LoginMethod = m.LoginMethod
	to.GrpcRequestMetadata = m.GrpcRequestMetadata
	to.ErrorMessage = m.ErrorMessage
	if posthook, ok := interface{}(m).(LoginLogsWithAfterToORM); ok {
		err = posthook.AfterToORM(ctx, &to)
	}
	return to, err
}
...
// ToPB runs the BeforeToPB hook if present, converts the fields of this
// object to PB format, runs the AfterToPB hook, then returns the PB object
func (m *LoginLogsORM) ToPB(ctx context.Context) (LoginLogs, error) {
	to := LoginLogs{}
	var err error
	if prehook, ok := interface{}(m).(LoginLogsWithBeforeToPB); ok {
		if err = prehook.BeforeToPB(ctx, &to); err != nil {
			return to, err
		}
	}
	...
	to.GrpcRequestMetadata = m.GrpcRequestMetadata
	to.ErrorMessage = m.ErrorMessage
	if posthook, ok := interface{}(m).(LoginLogsWithAfterToPB); ok {
		err = posthook.AfterToPB(ctx, &to)
	}
	return to, err
}
...
// The following are interfaces you can implement for special behavior during ORM/PB conversions
// of type LoginLogs the arg will be the target, the caller the one being converted from

// LoginLogsBeforeToORM called before default ToORM code
type LoginLogsWithBeforeToORM interface {
	BeforeToORM(context.Context, *LoginLogsORM) error
}

// LoginLogsAfterToORM called after default ToORM code
type LoginLogsWithAfterToORM interface {
	AfterToORM(context.Context, *LoginLogsORM) error
}

// LoginLogsBeforeToPB called before default ToPB code
type LoginLogsWithBeforeToPB interface {
	BeforeToPB(context.Context, *LoginLogs) error
}

// LoginLogsAfterToPB called after default ToPB code
type LoginLogsWithAfterToPB interface {
	AfterToPB(context.Context, *LoginLogs) error
}
...
func DefaultApplyFieldMaskAuthentication(ctx context.Context, patchee *Authentication, patcher *Authentication, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Authentication, error) {
	...
	var err error
	var updatedLastChangePassword bool
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"IsQlola" {
			patchee.IsQlola = patcher.IsQlola
			continue
		}
		if !updatedLastChangePassword && strings.HasPrefix(f, prefix+"LastChangePassword.") {
			if patcher.LastChangePassword == nil {
				patchee.LastChangePassword = nil
				continue
			}
			if patchee.LastChangePassword == nil {
				patchee.LastChangePassword = &timestamppb.Timestamp{}
			}
			childMask := &field_mask.FieldMask{}
			for j := i; j < len(updateMask.Paths); j++ {
				if trimPath := strings.TrimPrefix(updateMask.Paths[j], prefix+"LastChangePassword."); trimPath != updateMask.Paths[j] {
					childMask.Paths = append(childMask.Paths, trimPath)
				}
			}
			if err := gorm1.MergeWithMask(patcher.LastChangePassword, patchee.LastChangePassword, childMask); err != nil {
				return nil, nil
			}
		}
		if f == prefix+"LastChangePassword" {
			updatedLastChangePassword = true
			patchee.LastChangePassword = patcher.LastChangePassword
			continue
		}
		if f == prefix+"PrevSalt" {
			patchee.PrevSalt = patcher.PrevSalt
			continue
		}
	}
	if err != nil {
...
// DefaultCreateLoginLogs executes a basic gorm create call
func DefaultCreateLoginLogs(ctx context.Context, in *LoginLogs, db *gorm.DB) (*LoginLogs, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeCreate_); ok {
		if db, err = hook.BeforeCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithAfterCreate_); ok {
		if err = hook.AfterCreate_(ctx, db); err != nil {
			return nil, err
		}
	}
...
type LoginLogsORMWithBeforeCreate_ interface {
	BeforeCreate_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterCreate_ interface {
	AfterCreate_(context.Context, *gorm.DB) error
}

func DefaultReadLoginLogs(ctx context.Context, in *LoginLogs, db *gorm.DB) (*LoginLogs, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeReadApplyQuery); ok {
		if db, err = hook.BeforeReadApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	if db, err = gorm1.ApplyFieldSelection(ctx, db, nil, &LoginLogsORM{}); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeReadFind); ok {
		if db, err = hook.BeforeReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
	ormResponse := LoginLogsORM{}
	if err = db.Where(&ormObj).First(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormResponse).(LoginLogsORMWithAfterReadFind); ok {
		if err = hook.AfterReadFind(ctx, db); err != nil {
			return nil, err
		}
	}
...
type LoginLogsORMWithBeforeReadApplyQuery interface {
	BeforeReadApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithBeforeReadFind interface {
	BeforeReadFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterReadFind interface {
	AfterReadFind(context.Context, *gorm.DB) error
}

func DefaultDeleteLoginLogs(ctx context.Context, in *LoginLogs, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	ormObj, err := in.ToORM(ctx)
	...
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeDelete_); ok {
		if db, err = hook.BeforeDelete_(ctx, db); err != nil {
			return err
		}
	}
	err = db.Where(&ormObj).Delete(&LoginLogsORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithAfterDelete_); ok {
		err = hook.AfterDelete_(ctx, db)
	}
	return err
}

type LoginLogsORMWithBeforeDelete_ interface {
	BeforeDelete_(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterDelete_ interface {
	AfterDelete_(context.Context, *gorm.DB) error
}

func DefaultDeleteLoginLogsSet(ctx context.Context, in []*LoginLogs, db *gorm.DB) error {
	if in == nil {
		return errors.NilArgumentError
	}
	var err error
	...
	for _, obj := range in {
		...
		keys = append(keys, ormObj.RequestId)
	}
	if hook, ok := (interface{}(&LoginLogsORM{})).(LoginLogsORMWithBeforeDeleteSet); ok {
		if db, err = hook.BeforeDeleteSet(ctx, in, db); err != nil {
			return err
		}
	}
	err = db.Where("request_id in (?)", keys).Delete(&LoginLogsORM{}).Error
	if err != nil {
		return err
	}
	if hook, ok := (interface{}(&LoginLogsORM{})).(LoginLogsORMWithAfterDeleteSet); ok {
		err = hook.AfterDeleteSet(ctx, in, db)
	}
	return err
}

type LoginLogsORMWithBeforeDeleteSet interface {
	BeforeDeleteSet(context.Context, []*LoginLogs, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterDeleteSet interface {
	AfterDeleteSet(context.Context, []*LoginLogs, *gorm.DB) error
}

// DefaultStrictUpdateLoginLogs clears / replaces / appends first level 1:many children and then executes a gorm update call
func DefaultStrictUpdateLoginLogs(ctx context.Context, in *LoginLogs, db *gorm.DB) (*LoginLogs, error) {
	if in == nil {
		return nil, fmt.Errorf("Nil argument to DefaultStrictUpdateLoginLogs")
	}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	lockedRow := &LoginLogsORM{}
	db.Model(&ormObj).Set("gorm:query_option", "FOR UPDATE").Where("request_id=?", ormObj.RequestId).First(lockedRow)
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeStrictUpdateCleanup); ok {
		if db, err = hook.BeforeStrictUpdateCleanup(ctx, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeStrictUpdateSave); ok {
		if db, err = hook.BeforeStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
	...
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithAfterStrictUpdateSave); ok {
		if err = hook.AfterStrictUpdateSave(ctx, db); err != nil {
			return nil, err
		}
	}
...
type LoginLogsORMWithBeforeStrictUpdateCleanup interface {
	BeforeStrictUpdateCleanup(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithBeforeStrictUpdateSave interface {
	BeforeStrictUpdateSave(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterStrictUpdateSave interface {
	AfterStrictUpdateSave(context.Context, *gorm.DB) error
}

// DefaultPatchLoginLogs executes a basic gorm update call with patch behavior
func DefaultPatchLoginLogs(ctx context.Context, in *LoginLogs, updateMask *field_mask.FieldMask, db *gorm.DB) (*LoginLogs, error) {
	if in == nil {
		return nil, errors.NilArgumentError
	}
	var pbObj LoginLogs
	var err error
	if hook, ok := interface{}(&pbObj).(LoginLogsWithBeforePatchRead); ok {
		if db, err = hook.BeforePatchRead(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if hook, ok := interface{}(&pbObj).(LoginLogsWithBeforePatchApplyFieldMask); ok {
		if db, err = hook.BeforePatchApplyFieldMask(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	if _, err := DefaultApplyFieldMaskLoginLogs(ctx, &pbObj, in, updateMask, "", db); err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&pbObj).(LoginLogsWithBeforePatchSave); ok {
		if db, err = hook.BeforePatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
	pbResponse, err := DefaultStrictUpdateLoginLogs(ctx, &pbObj, db)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(pbResponse).(LoginLogsWithAfterPatchSave); ok {
		if err = hook.AfterPatchSave(ctx, in, updateMask, db); err != nil {
			return nil, err
		}
	}
...
type LoginLogsWithBeforePatchRead interface {
	BeforePatchRead(context.Context, *LoginLogs, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsWithBeforePatchApplyFieldMask interface {
	BeforePatchApplyFieldMask(context.Context, *LoginLogs, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsWithBeforePatchSave interface {
	BeforePatchSave(context.Context, *LoginLogs, *field_mask.FieldMask, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsWithAfterPatchSave interface {
	AfterPatchSave(context.Context, *LoginLogs, *field_mask.FieldMask, *gorm.DB) error
}

// DefaultPatchSetLoginLogs executes a bulk gorm update call with patch behavior
func DefaultPatchSetLoginLogs(ctx context.Context, objects []*LoginLogs, updateMasks []*field_mask.FieldMask, db *gorm.DB) ([]*LoginLogs, error) {
	if len(objects) != len(updateMasks) {
		return nil, fmt.Errorf(errors.BadRepeatedFieldMaskTpl, len(updateMasks), len(objects))
	}

	results := make([]*LoginLogs, 0, len(objects))
	for i, patcher := range objects {
		pbResponse, err := DefaultPatchLoginLogs(ctx, patcher, updateMasks[i], db)
		if err != nil {
			return nil, err
		}
...
// DefaultApplyFieldMaskLoginLogs patches an pbObject with patcher according to a field mask.
func DefaultApplyFieldMaskLoginLogs(ctx context.Context, patchee *LoginLogs, patcher *LoginLogs, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*LoginLogs, error) {
	if patcher == nil {
		return nil, nil
	} else if patchee == nil {
		return nil, errors.NilArgumentError
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"LoginMethod" {
			patchee.LoginMethod = patcher.LoginMethod
			continue
		}
		if f == prefix+"GrpcRequestMetadata" {
			patchee.GrpcRequestMetadata = patcher.GrpcRequestMetadata
			continue
		}
...
// DefaultListLoginLogs executes a gorm list call
func DefaultListLoginLogs(ctx context.Context, db *gorm.DB) ([]*LoginLogs, error) {
	in := LoginLogs{}
	ormObj, err := in.ToORM(ctx)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeListApplyQuery); ok {
		if db, err = hook.BeforeListApplyQuery(ctx, db); err != nil {
			return nil, err
		}
	}
	db, err = gorm1.ApplyCollectionOperators(ctx, db, &LoginLogsORM{}, &LoginLogs{}, nil, nil, nil, nil)
	if err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithBeforeListFind); ok {
		if db, err = hook.BeforeListFind(ctx, db); err != nil {
			return nil, err
		}
	}
	db = db.Where(&ormObj)
	db = db.Order("request_id")
	ormResponse := []LoginLogsORM{}
	if err := db.Find(&ormResponse).Error; err != nil {
		return nil, err
	}
	if hook, ok := interface{}(&ormObj).(LoginLogsORMWithAfterListFind); ok {
		if err = hook.AfterListFind(ctx, db, &ormResponse); err != nil {
			return nil, err
		}
	}
	pbResponse := []*LoginLogs{}
	for _, responseEntry := range ormResponse {
		temp, err := responseEntry.ToPB(ctx)
		if err != nil {
			return nil, err
...
type LoginLogsORMWithBeforeListApplyQuery interface {
	BeforeListApplyQuery(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithBeforeListFind interface {
	BeforeListFind(context.Context, *gorm.DB) (*gorm.DB, error)
}
type LoginLogsORMWithAfterListFind interface {
	AfterListFind(context.Context, *gorm.DB, *[]LoginLogsORM) error
}
