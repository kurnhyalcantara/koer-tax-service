...
for folder in "$@"; do
  ...
  find "$TARGET_FOLDER" -name "*.proto" -print0 | while IFS= read -r -d '' entry; do
    ...
    --go_out="$OUTPUT_FOLDER" --go_opt=paths=source_relative \
      ...
      --validate_out="lang=go,paths=source_relative:$OUTPUT_FOLDER" \
      "$entry"
  done
  ...
  if [ -f "./protogen/koer-tax-service/tax_service_api.swagger.json" ]; then
    echo "Moving swagger file from koer-tax-service"
    mkdir -p "./www"
    mv ./protogen/koer-tax-service/tax_service_api.swagger.json ./www/swagger.json
  fi
done
